...
class Test:
    ...
    def test_inv_delete_cascade(self):
        """
        test delete cascade of invoice
        :return:
        """
        ...
        with self.session.no_autoflush:
            self.session.delete(self.session.query(Invoice)[0])
        assert 11 == self.session.query(Invoice).count()
        assert 22 == self.session.query(Iitem).count()
        assert 44 == self.session.query(Citem).count()
...
class Test2:
    ...
    def test_inv_item_delete_cascade(self):
        """
        test delete cascade of invoice item
        setup not called again????
        :return:
        """
        assert 12 == self.session.query(Invoice).count()
        assert 24 == self.session.query(Iitem).count()
        assert 48 == self.session.query(Citem).count()
        with self.session.no_autoflush:
            self.session.delete(self.session.query(Iitem)[0])
        assert 12 == self.session.query(Invoice).count()
        assert 23 == self.session.query(Iitem).count()
        assert 46 == self.session.query(Citem).count()
