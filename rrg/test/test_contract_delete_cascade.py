...
class Test:
    ...
    def test_contract_delete_cascade(self):
        """
        test delete cascade of contract
        :return:
        """
        ...
        with self.session.no_autoflush:
            self.session.delete(self.session.query(Contract)[0])
        ...
        assert 11 == self.session.query(Contract).count()
        assert 4 == self.session.query(Employee).count()
        assert 2 == self.session.query(Client).count()
        assert 44 == self.session.query(ContractItemCommItem).count()
