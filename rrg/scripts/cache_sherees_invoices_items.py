...
from rrg.sherees_commissions import \
    cache_invoices_items as cache_invoices_items_routine
...
parser.add_argument(
    '--cache', dest='cache', action='store_true', help='if true use xml for sales persons payment lookups')
...
def cache_invoices_items_ep():
    ...
    cache_invoices_items_routine(args.datadir, session, sa_sheree(session), args.cache)
    session.commit()
...
def cache_invoices_items(cache):
    session = session_maker(
        app.config['MYSQL_USER'], app.config['MYSQL_PASS'], app.config['MYSQL_SERVER_PORT_3306_TCP_ADDR'],
        app.config['MYSQL_SERVER_PORT_3306_TCP_PORT'], app.config['DB'])
    print('Caching Sherees Invoices Items')
    cache_invoices_items_routine(app.config['DATADIR'], session, sa_sheree(session), cache)
    session.commit()
