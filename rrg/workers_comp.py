...
def wc_report(session, args):
    ...
    invoices = session.query(Invoice).\
        ...
        filter(Invoice.posted == 1, Invoice.voided == 0, Invoice.mock == 0, Invoice.cleared == 1, Invoice.prcleared == 1,
        Invoice.period_start >= args.start_date, Invoice.period_end < args.end_date) 
    employees = set()
    states = set()
    for inv in invoices:
